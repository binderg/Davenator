...
function addItem(type){
		...
		$columns[l] = { id:$idIncrement, name:"", minutes:null, beforeAfter: "before", time: "", text:""};
		if (type=="rule"){
			$columns[l].type="rule";
		}
		else{ $columns[l].type="textbox";}
		console.log($columns);
...
let parshiyosObj = {};
let parshaURL = 'https://www.hebcal.com/hebcal?v=1&cfg=json&year=now&month=x&s=on&leyning=off'
...
async function parshiyos() {
		...
		if (res.ok) {	
			...
			parshiyosObj[element.title.substr(9)] = {};
			parshiyosObj[element.title.substr(9)].unformatted = element.date;
			//Reformat date
			let dateArray = element.date.split('-')
			//Remove "Parshat" and add to parshiyos array
			let date = `${dateArray[1]}/${dateArray[2]}/${dateArray[0]}`;
			parshiyosObj[element.title.substr(9)].formatted = date;
			});				
		} else {
...
<main>
	...
	<a on:click = {() => addItem("rule")} class="waves-effect waves-light btn"><i class="material-icons left">code</i>Add Rule Column</a>
	<a on:click = {() => addItem("textbox")} class="waves-effect waves-light btn"><i class="material-icons left">text_fields</i>Add Text Column</a>
	<a on:click = {save} class="waves-effect waves-light btn"><i class="material-icons left">cloud</i>Save</a>
